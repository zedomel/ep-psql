...
public class QuadTree<T> {
	...
	/**
	 * Add the value of an (x, y) point within the quad-tree.
	 *
	...
	public Point<T>[] getKeys() {
		...
		return (Point<T>[]) arr.toArray(new Point[arr.size()]);
	}
	...
	public T[] getValues() {
		...
		return (T[]) arr.toArray(new Object[arr.size()]);
	}
	...
	public QuadTree<T> clone() {
		...
		this.traverse(this.root_, new Func<T>() {
			@Override
			public void call(QuadTree<T> quadTree, Node<T> node) {
				clone.add(node.getPoint().getX(), node.getPoint().getY(), node.getPoint().getValue());
			}
		});
	 ...
	 * @private
	 */
	public void traverseAll(Node<T> node, Func<T> func) {
		func.call(this, node);
	...
	public static void main(String[] args) {
		QuadTree<String> tree = new QuadTree<>(0, 0, 128, 128);
		...
		tree.traverseAll(tree.root_, (quadtree, node) -> {
